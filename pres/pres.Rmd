...
- understanding the relevant/slated oprations helps
    ...
    \node [name=t2, treenode, above right=0.6875cm and 2cm of t1, fill=proc2,
        ...
        child { node [treenode, fill=proc2] {$\ast$ \nodepart{two} $\ast$}
            child { node [treenode, fill=proc2] {$\ast$ \nodepart{two} $\ast$}
                ...
                child [sibling distance=1cm] { node [treenode, fill=proc2] { \_ \nodepart{two} \_ } }
            }
            child { node [treenode, fill=proc2] {$\ast$ \nodepart{two} $\ast$}
                child [sibling distance=1cm] { node [treenode, fill=proc2] { \_ \nodepart{two} \_ } }
                child [sibling distance=1cm] { node [treenode, fill=proc2] { \_ \nodepart{two} \_ } }
...
dep.latches.emplace(dkey, new boost::latch(ctts::HWCONC));
// make sure we have a valid _orig if not monotonic
if (!ctts::is_monotonic(imap)) {
    // locked this->_data
...
if (!ctts::is_monotonic(dep_tracker.imaps[0])) {
...
// foreach
auto ret1 = cont_inp.foreach(contentious::mult<double>, 2);
auto ret2 = ret1->foreach(contentious::mult<double>, cont_other);
contentious::tp.finish();
```
...
```C++
template <typename T>
cont_vector<T> cont_vector<T>::reduce(const ctts::op<T> op)
{
...
template <typename T>
template <int... Offs>
std::shared_ptr<cont_vector<T>> cont_vector<T>::stencil(const std::vector<T> &coeffs)
{
...
void stencil_splt(cont_vector<T> &cont, cont_vector<T> &dep, const uint16_t p)
{
...
- fundamental problem has been "reduced" to partitioning and managing the input and output index sets, and the mapping between them
